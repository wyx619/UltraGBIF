...
This Tutorial demonstrates the usage of UltraGBIF parsing and merging Saxifraga (<https://doi.org/10.15468/dl.bythb4>) occurrence records from GBIF as an example.

## Import records in UltraGBIF

This tutorial uses *Saxifraga* occurrence records downloaded from GBIF.

This module also utilizes [`EnumOccurrence`](https://gbif.github.io/gbif-api/apidocs/org/gbif/api/vocabulary/OccurrenceIssue.html) Issue values to transform these issues into a structured format, enabling systematic quality assessment through individualized issue tracking and standardized classification.
...
`occ_import` includes `occ` for initial occurrence records, `occ_gbif_issue` for GBIF issues and its `summary`, and runtime.
...
`dictionary` includes `my_dictionary` for checked collectors name and `ref_dictionary` for referred dictionary, and runtime.
...
`collection_key` includes `occ_collectorsDictionary` for input records updated with standardized fields and generated keys and `CollectorsDictionary_add` for newly encountered collectors for dictionary expansion and runtime.
...
`voucher` includes the full flagged digital voucher of occurrence records which names `occ_digital_voucher` and its `summary`, and runtime.
...
After simplification, dynamic maps with different layers are rendered by mapview[@mapview] with WGSRPD information legend. Symbol size is controlled by the `cex` parameter. Moreover, they support zoom functionality for spatial resolution adjustment and enables interactive selection of individual coordinates to display their full attributes.

```{r map_records, eval=FALSE, include=T}
map_records(refined_records = refined_records,
                     precision = 2, 
                     cex = 4)
```
...
richness <- plot_richness(refined_records = refined_records,main = 'Saxifraga Richness')
```
